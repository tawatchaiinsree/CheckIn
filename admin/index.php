<?php
    ...
    $result = $conn->query("SELECT count(*) count FROM user WHERE permission ='user'");
    if ($result->num_rows > 0) {while($row = $result->fetch_assoc()) {$count = $row['count'];}}else{$count = 0;}
    $result = $conn->query("SELECT count(*) count FROM check_time WHERE date='".date("Y-m-d")."'");
    if ($result->num_rows > 0) {while($row = $result->fetch_assoc()) {$come_count = $row['count'];}}else{$count = 0;}
?>
...
<div class="left form-inline" style="display: inline; margin:auto; width:100%;">
    ...
    <table id="example" class="table table-striped table-bordered top tb-center top" style="width:100% justify-content: center; align-content: center;">
        <thead>
